...
def test_worker_upload_failure_marks_job_failed_and_records_metrics(monkeypatch):
    ...
    failed_before = worker_jobs_failed_total.labels(
        stage="upload", **metric_labels
    )._value.get()
    minio_before = worker_minio_upload_failures_total._value.get()
    ...
    assert worker_minio_upload_failures_total._value.get() >= minio_before + 1
    assert (
        worker_jobs_failed_total.labels(stage="upload", **metric_labels)._value.get()
        >= failed_before + 1
    )
    assert audit_events
