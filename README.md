...
Current limitations in the checked-in workflow:

- it uses `working-directory: saasguard`, which should be verified against the real repository root in GitHub
- it does not currently create `.env` from `.env.example`
- it does not currently create `frontend/.env` from `frontend/.env.example`
- it does not currently build the frontend in CI
